...
export function SpotlightBar() {
  ...
  const top = sorted[0];
  const second = sorted[4]; // 5th agent for variety
      ...
      <div className="flex gap-3 overflow-x-auto pb-1 -mx-1 px-1">
        {top && (
          <div className="flex items-center gap-3 bg-[#0c0c0c] border border-white/[0.06] rounded-2xl px-4 py-3 min-w-[210px] shrink-0 hover:border-white/[0.1] transition-colors">
            ...
            <div>
              <div className="text-[12px] font-semibold text-white">{top.name}</div>
              <div className="text-[13px] text-zinc-600">Upgraded to Gold tier</div>
            </div>
          </div>
        )}
        {second && (
          <div className="flex items-center gap-3 bg-[#0c0c0c] border border-white/[0.06] rounded-2xl px-4 py-3 min-w-[210px] shrink-0 hover:border-white/[0.1] transition-colors">
            ...
            <div>
              <div className="text-[12px] font-semibold text-white">{second.name}</div>
              <div className="text-[13px] text-zinc-600">ZK proof verified</div>
            </div>
          </div>
        )}
        {[1, 2, 3].map((i) => (
          <div key={i} className="flex items-center justify-center gap-1.5 border border-dashed border-white/[0.06] rounded-2xl px-4 py-3 min-w-[120px] shrink-0">
            ...
            <span className="text-[13px] text-zinc-700">Available</span>
          </div>
        ))}
      </div>
    </section>
