...
export default function AgentProfilePage() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-[11px] font-mono text-zinc-500 tracking-widest animate-pulse">LOADING_AGENT_PASSPORT...</div>
      </div>
    );
  }
  ...
  if (!agent) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-[11px] font-mono text-red-500 tracking-widest">AGENT_NOT_FOUND</div>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="flex items-start justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] uppercase tracking-[2px] text-[var(--text-secondary)] mt-1">
              ID: {agent.public_key} · REGISTERED: {new Date(agent.created_at).toLocaleDateString()}
            </p>
          </div>
        </div>
        <TruvaButton variant="outlined" className="text-[9px]">EXPORT_PASSPORT</TruvaButton>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
        <div className="bg-[var(--bg-card)] border border-[var(--accent-green)] rounded-[2px] p-5 flex flex-col items-center justify-center text-center">
          ...
          <div className="text-[10px] uppercase tracking-[2px] text-[var(--accent-green)] mb-1">TRUST_SCORE</div>
          <div className="text-[36px] font-bold">{agent.trust_score}</div>
          <div className="text-[9px] text-[var(--text-muted)] mt-1">TIER: {tierLabel.toUpperCase()}</div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div className="bg-[var(--bg-card)] border border-[var(--border-default)] rounded-[2px] p-5">
          <h3 className="text-[11px] uppercase tracking-[2px] font-bold mb-3">OPERATOR_DETAILS</h3>
          <div className="space-y-2">
            ...
            {agent.pda_address && (
              <div className="flex justify-between text-[11px]">
                ...
                <span className="font-mono text-[10px]">{agent.pda_address}</span>
              </div>
            )}
            <div className="flex justify-between text-[11px]">
        ...
        <div className="bg-[var(--bg-card)] border border-[var(--border-default)] rounded-[2px] p-5">
          <h3 className="text-[11px] uppercase tracking-[2px] font-bold mb-3">CHAIN_SUPPORT</h3>
          ...
          {agent.description && (
            <div className="mt-4 pt-3 border-t border-[var(--border-subtle)]">
              <h4 className="text-[10px] uppercase tracking-[2px] text-[var(--text-muted)] mb-2">DESCRIPTION</h4>
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border-default)] rounded-[2px] p-5">
        <h3 className="text-[11px] uppercase tracking-[2px] font-bold mb-4">TRUST_METRICS</h3>
