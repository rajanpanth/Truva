...
const navItems: NavItem[] = [
  ...
  { label: 'VAULT_OPS', href: '#', icon: <Shield size={15} /> },
  { label: 'REPUTATION_LOGS', href: '/trustgate-logs', icon: <Activity size={15} /> },
  { label: 'API_KEYS', href: '#', icon: <Key size={15} /> },
  { label: 'SYSTEM_STATUS', href: '#', icon: <Cpu size={15} /> },
];
...
export function ShellASidebar() {
  ...
  return (
    <aside className="fixed top-12 left-0 w-[210px] h-[calc(100vh-48px-36px)] bg-[var(--bg-input)] border-r border-[var(--border-default)] flex flex-col z-40">
      {/* Node profile */}
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <div className="flex items-center gap-1">
              ...
              <span className="text-[13px] text-[var(--accent-green)]">SOLANA_MAINNET</span>
            </div>
          </div>
        </div>
      </div>
        ...
        {isSdkPage ? (
          sdkNavItems.map((group) => (
            <div key={group.section}>
              <div className="px-4 pt-4 pb-2 text-[12px] uppercase tracking-[2px] text-[var(--text-muted)]">
                {group.section}
              </div>
              {group.items.map((item) => {
                ...
                return (
                  <Link
                    key={item.label}
                    href={item.href}
                    className={`flex items-center gap-2.5 px-4 py-2.5 text-[13px] uppercase tracking-[1px] transition-colors ${
                      isActive
      ...
      <div className="p-4 border-t border-[var(--border-default)] space-y-3">
        <Link href="/register">
          <TruvaButton variant="outlined" className="w-full text-[12px]">
            DEPLOY_NEW_AGENT
          </TruvaButton>
        </Link>
        <div className="space-y-1">
          <Link
            href="/sdk-docs"
            className="flex items-center gap-2 px-1 py-1 text-[13px] text-[var(--text-muted)] hover:text-[var(--text-secondary)] transition-colors"
          >
